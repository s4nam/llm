...
export default function MonetizationManager({
  ...
  const [trialHours, setTrialHours] = useState(pricing?.trial_hours ?? 72);
  const [graceHours, setGraceHours] = useState(pricing?.trial_grace_hours ?? 48);
  ...
  async function addCoupon() {
    ...
    if (data.error) {
      ...
      setMsg("Kupon dibuat.");
      setCouponForm({ code: "", type: "percent", value: 10, maxUses: 1 });
  ...
  async function setMember(userId: string, days: number, note: string) {
    setSaving(true);
    setMsg(null);
    const res = await fetch("/api/admin/monetization", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "set-member", userId, days, note }),
    });
    const data = await res.json();
    setSaving(false);
    setMsg(data.error ? `Gagal: ${data.error}` : "Status member diperbarui.");
  }
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input type="number" value={graceHours} onChange={(e) => setGraceHours(Number(e.target.value))} className={inputCls} />
          </div>
        </div>
        <button
          onClick={savePricing}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {coupons.map((c) => (
            ...
            >
              {c.code} — {c.discount_type === "percent" ? `${c.discount_value}%` : `Rp ${c.discount_value.toLocaleString("id-ID")}`}
              {!c.active && " (nonaktif)"}
            </span>
          ))}
      ...
      <section className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-6 py-4">
          ...
          <p className="text-sm text-slate-500">
            Set status manual untuk kompensasi, koreksi, atau reset trial.
          </p>
        </div>
              ...
              <tr key={m.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-3">
                  {m.is_member ? (
                    <span className="rounded-full bg-success/10 px-2 py-0.5 text-xs text-success">
                      Member {m.member_expires_at ? `s/d ${new Date(m.member_expires_at).toLocaleDateString("id-ID")}` : ""}
                    </span>
                  ) : m.trial_used ? (
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() => setMember(m.id, 30, "Manual admin")}
                      disabled={saving}
                    ...
                    <button
                      onClick={() => setMember(m.id, 365, "Manual admin")}
                      disabled={saving}
                      className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium hover:bg-slate-50"
                    >
                      +1 tahun
                    ...
                    <button
                      onClick={async () => {
                        ...
                        const res = await fetch("/api/admin/monetization", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({ action: "reset-trial", userId: m.id }),
                        });
                        setSaving(false);
                        const d = await res.json();
                        setMsg(d.error ? `Gagal: ${d.error}` : "Trial direset.");
                      }}
                      disabled={saving}
